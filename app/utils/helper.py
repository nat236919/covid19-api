...
# Encapsulated the Helper_functions
...
class Helping_tools:
    ...
    def helper_get_latest_data_url(base_url: str) -> str:
        ...
        while requests.get(latest_base_url).status_code == 404:
            current_datetime = datetime.strftime(
                datetime.utcnow() - timedelta(time_delta), time_format)
            latest_base_url = base_url.format(current_datetime)
            time_delta += 1
    ...
    def helper_lookup_country(country: str) -> str:
        """ Look up a country name from a country code """
        country_name = pycountry.countries.lookup(
            country).name  # Select the first portion of str when , is found
        if ',' in country_name:
