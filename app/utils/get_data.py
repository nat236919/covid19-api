...
from .helper import SingletonHelper
...
class DailyReports:
    def __init__(self) -> None: 
        self.latest_base_url = sHelper.helper_get_latest_data_url(JHU_CSSE_FILE_PATHS['BASE_URL_DAILY_REPORTS'])
        self.latest_base_US_url = sHelper.helper_get_latest_data_url(JHU_CSSE_FILE_PATHS['BASE_URL_DAILY_REPORTS_US'])
    ...
    def get_data_daily_reports(self, US: bool = False) -> pd.DataFrame:
        ...
        df = sHelper.helper_df_cols_cleaning(df, concerned_columns, int)
...
class DataTimeSeries:
    ...
    def _clean_timeseries_dataframe(self, df: pd.DataFrame, US: bool = False) -> pd.DataFrame:
        df_cleaned = sHelper.helper_df_cleaning(df) # main pre-processing
        if US:
            df_cleaned = sHelper.helper_df_cols_cleaning(df_cleaned, ['Lat', 'Long_'], float)
        return df_cleaned
