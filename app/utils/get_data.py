...
class DataProvider:
    def __init__(self, name, URL, baseURL, dailyURL, USdailyURL):
        self.name = name
        self.URL = URL
        self.baseURL = baseURL
        self.dailyURL = dailyURL
        self.USdailyURL = USdailyURL
...
# Get Lookup table
    def get_data_lookup_table() -> Dict[str, str]:
        ...
        URL = name['BASE_URL_LOOKUP_TABLE']
        lookup_df = pd.read_csv(URL)[['iso2', 'Country_Region']]
...
# Get data from daily reports
    def get_data_daily_reports() -> pd.DataFrame:
        ...
        baseURL = helper_get_latest_data_url(name['BASE_URL_DAILY_REPORTS'])
        # Extract the data
...
# Get data from daily reports (USA)
    def get_data_daily_reports_us() -> pd.DataFrame:
        ...
        USdailyURL = helper_get_latest_data_url(name['BASE_URL_DAILY_REPORTS_US'])

        # Extract the data
        #df = pd.read_csv(latest_base_url)
        df = pd.read_csv(USdailyURL)        
...
# Get data from time series
    def get_data_time_series() -> Dict[str, pd.DataFrame]:
        ...
        for category in JHU_CSSE_FILE_PATHS['CATEGORIES']:
            ...
            url = name['BASE_URL_TIME_SERIES'].format(category)
    ...
    def get_US_time_series() -> Dict[str, pd.DataFrame]:
        ...
        for category in JHU_CSSE_FILE_PATHS['CATEGORIES'][:-1]:
            ...
            url = name['BASE_URL_US_TIME_SERIES'].format(category)
