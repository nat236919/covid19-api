...
from .DataTimeSeries_Interface import IDataTimeSeries
from .DataTimeSeries_Adapter import DataTimeSeriesAdapter
...
class DataTimeSeries(IDataTimeSeries):
    ...
    def _clean_timeseries_dataframe(self, df: pd.DataFrame, US: bool = False) -> pd.DataFrame:
        df_cleaned = DataTimeSeriesAdapter.helper_df_cleaning(df) # main pre-processing
        if US:
            df_cleaned = DataTimeSeriesAdapter.helper_df_cols_cleaning(df_cleaned, ['Lat', 'Long_'], float)
        return df_cleaned
