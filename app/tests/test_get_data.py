...
# Import libraries
import pandas as pd
from ..utils.get_data import DailyReports, DataGetter, get_data
...
def test_get_data_lookup_table() -> None:
    result = data_getter.get_data_lookup_table()
    assert len(result) > 0
...
def test_get_data_time_series() -> None:
    result = data_getter.get_data_time_series()
    assert len(result) > 0
    assert isinstance(result, dict) is True
...
def test_get_US_time_series() -> None:
    result = data_getter.get_US_time_series()
    assert len(result) > 0
    assert isinstance(result, dict) is True
