...
def test_helper_df_cleaning() -> None:
    df = DF.copy()
    res_df = sHelper.helper_df_cleaning(df)
...
def test_helper_df_cols_cleaning() -> None:
    df = DF.copy()
    res_df = sHelper.helper_df_cols_cleaning(df, ['col_2'], str)
    res_col_1 = [i for i in res_df['col_1'].values]
    res_col_2 = [i for i in res_df['col_2'].values]
    assert isinstance(res_df, pd.DataFrame) is True
    assert res_col_1 == [i for i in DF['col_1'].values]
...
def test_helper_get_latest_data_url() -> None:
    assert isinstance(sHelper.helper_get_latest_data_url(file_paths.JHU_CSSE_FILE_PATHS['BASE_URL_LOOKUP_TABLE']), str)
    assert isinstance(sHelper.helper_get_latest_data_url(file_paths.JHU_CSSE_FILE_PATHS['BASE_URL_DAILY_REPORTS']), str)
    assert isinstance(sHelper.helper_get_latest_data_url(file_paths.JHU_CSSE_FILE_PATHS['BASE_URL_DAILY_REPORTS_US']), str)
